...
namespace DonkeyKong
{
	Ladder::Ladder(Vec2 pos, unsigned int length_multiplier) : GameObject(pos, 15, 14, "Assets/ladder.png")
	{
		...
		colliderPtr = std::make_shared<Collider>(*this, std::move(rect));

		if (length_multiplier > 1)
		{
			colliderAtDown = std::make_shared<Collider>(*this, Rect{ rect.x, (int) pos.y - 14, rect.w, rect.h });
		}
	}
	...
	void Ladder::Draw(const Renderer& renderer) const
	{
		...
		if (DrawCollider && colliderAtDown != nullptr)
		{
			colliderAtDown->Draw(renderer);
		}
	}
}
