...
{
	const Vec2 Map::PLATFORM_START_POSITION = { 30, 100 };
	const Vec2 Map::PLATFORM_END_POSITION = { 580, 500 };
	const Vec2 Map::PLATFORM_OFFSET = { 36, 60 };
	const Vec2 Map::ENEMY_OFFSET_FROM_PLATFORM = { 40, -32 };
	const Vec2 Map::PLAYER_OFFSET_FROM_PLATFORM = { 10, -15 };
	const Vec2 Map::LADY_OFFSET_FROM_HER_PLATFORM = { 2, -23 };
	const Vec2 Map::LADY_PLATFORM_OFFSET_FROM_ENEMY = { 50, -20 };
	const Uint32 Map::PLAYER_PLATFORM_LENGTH = 170;
	const Uint32 Map::ENEMY_PLATFORM_LENGTH = 170;
	const Uint32 Map::LADY_PLATFORM_LENGTH = 72;
	const int Map::Y_PLATFORM_GRADIENT = 1;
	const int Map::PLATFORM_SUBTRACT = 2;
	const float Map::PLATFORM_ROTATION = 1;
	const float Map::SINGLE_LADDER_SIZE = 14;
	...
	void Map::Draw(const Renderer& renderer) const
	{
	...
	void Map::Update(const Timer& timer)
	{
		...
		if (hero->IsDead())
		{
			if(timeToStopUpdate<= 0) 
				return;
			else 
				timeToStopUpdate -= timer.DeltaTime();
		}
	...
	const Vec2 Map::CreateHorizontal(Vec2 startPos, const int xEnd, const bool createLadder, const int yOffset, const float rotation)
	{
	...
	void Map::CreateLadders(const int ladder_size)
	{
